  ...
  function FP(S, sess, onExit) {
    ...
    wrap.innerHTML =
      ...
      '<div id="fphud">' +
        ...
        '<div class="fp-bottom">' +
          ...
          '<div id="fp-tools"></div>' +
        '</div>' +
        '<div class="fp-help">滑鼠移動＝看　WASD＝走　左鍵/空白鍵＝挖　1~8＝換工具　T＝換人　F＝燈　P＝抽水　G＝架坑木　M＝地圖　Z＝過夜　X＝開槍（打岩壁、威嚇全場、擊斃準星裡的人）　Q＝收工</div>' +
      '</div>' +
      '<div id="fp-touch">' +
        '<div id="joy"><i></i></div>' +
        ...
        '<div id="fp-actions">' +
          ...
          (hasWorking(S, 'shotgun') ? '<button data-k="X" class="quit">🔫開槍</button>' : '') +
    ...
    function npcInSight() {
      ...
      for (const n of sess.npcs) {
        ...
        const maxD = flee ? 14 : 8;
        const cone = (flee ? 0.16 : 0.09) * (isTouch ? 1.9 : 1);
    ...
    /* ---- 槍擊岩壁：火花四濺、岩石碎裂，打出的料裂到不值錢 ---- */
    function shootWall() {
      const tgt = aim(sess, 10);
      if (!tgt || tgt.t === BED) return false;
      const lv = sess.lv, i = idx(tgt.x, tgt.y);
      const T = TYPE[lv.map[i]];
      lv.hp[i] -= 34 / T.hard;
      // 彈著火花
      for (let k = 0; k < 22; k++) particles.push({
        x: cv.width / 2 + (Math.random() - 0.5) * 60,
        y: cv.height / 2 + (Math.random() - 0.5) * 60,
        vx: (Math.random() - 0.5) * 700, vy: (Math.random() - 0.8) * 500,
        life: 0.25 + Math.random() * 0.4, max: 0.65,
        size: 1 + Math.random() * 2.5, c: [255, 200, 110], glow: true
      });
      if (lv.hp[i] <= 0) {
        lv.map[i] = OPEN; sess.dugCount++;
        const st = lv.nodes[i];
        if (st) {
          J.addMiningCrack(st, 0.55 + Math.random() * 0.3);
          st.note = '被槍打碎過';
          sess.stones.push(st);
          delete lv.nodes[i];
          say('💥 一槍轟開岩壁 — 裡面有料！可惜被打得稀爛…');
        } else say('💥 岩壁被轟開一個洞。');
        markSeen(sess);
      }
      return true;
    ...
    function menace() {
      ...
      say('🔫 砰！' + (hitWall ? '彈丸打在岩壁上火花四濺' : '槍聲在坑道裡炸開') + ' — 沒人敢停手了（士氣 -15、名聲 -3）');
